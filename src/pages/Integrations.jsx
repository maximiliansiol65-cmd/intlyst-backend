...
export default function Integrations() {
  ...
  const [connectedMap, setConnectedMap] = useState({}); // type → true
  ...
  const fetchStatus = useCallback(async () => {
    ...
    try {
      ...
      const map = {};
      data.forEach(item => { if (item.is_active) map[item.integration_type] = item.configured_fields; });
      setConnectedMap(map);
    } catch {
              ...
              <div key={integration.type} style={{
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: "var(--text-xs)", color: "#16a34a", marginTop: 1, display: "flex", alignItems: "center", gap: 3 }}>
                    <IcoCheck /> Aktiv — {(connectedMap[integration.type] || []).length} Felder konfiguriert
                  </div>
                </div>
                <button
                  className="btn btn-ghost btn-sm"
