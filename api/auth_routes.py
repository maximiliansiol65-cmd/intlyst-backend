...
def _ensure_default_workspace_for_user(db: Session, user: User) -> int:
    membership = (
        ...
        .filter(WorkspaceMembership.user_id == user.id, WorkspaceMembership.is_active == True)
        .first()
    )
    if membership:
    ...
    preferred_name = user.company or user.name or f"{user.email.split('@')[0]} Workspace"
    workspace = Workspace(
